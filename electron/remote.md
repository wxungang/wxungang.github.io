...
## 原理
- 本质是通过ipc通信获取
    - 同步ipc通信！`ipcRendererInternal.sendSync()`
    - 普通数据类型: 直接获取实时的同步ipc通信返回值
    - 函数：重写函数，当函数被调用时，返回实时的同步ipc通信返回值
    - 对象：遍历对象 & 重写`get/set`,这样在get/set操作时，调用实时的同步ipc通信更新主进程数据。
    - 数组：遍历数组，具体处理逻辑看数组元素的数据类型！
